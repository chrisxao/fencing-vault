...
export function LabelerPage() {
  ...
  const seek = (ms: number) => { const next = Math.max(0, Math.min(duration, ms)); setCurrentMs(next); if (videoRef.current) videoRef.current.currentTime = next / 1_000; };
    ...
    <div className="labeler-workspace">
      ...
      <aside className="inspector-column">
        {selected ? <><div className="inspector-title"><div><p className="eyebrow">PHRASE {selected.ordinal} · REV {selected.revision}</p><h2>{formatTime(selected.startMs)} — {formatTime(selected.endMs)}</h2></div><span className={`award-mark ${selected.award}`}>{selected.award === 'none' ? 'NO TOUCH' : selected.award === 'unknown' ? 'UNCALLED' : `${selected.award.toUpperCase()} +1`}</span></div>
          ...
          {tab === 'geometry' && <PoseEditor bout={bout} currentMs={currentMs} onSaved={() => { notify('Pose keyframe saved'); void load(); }} />}
        </> : <div className="no-selection"><span>⌁</span><h2>Select a phrase</h2><p>Choose a phrase below the video, or mark its start and end.</p></div>}
      </aside>
    </div>
...
function VideoStage({ bout, ingestion, currentMs, duration, videoRef, onTime, onSeek, onFrame }: { bout: BoutDetail; ingestion: IngestionJobRecord | null; currentMs: number; duration: number; videoRef: React.RefObject<HTMLVideoElement | null>; onTime: (value: number) => void; onSeek: (value: number) => void; onFrame: (amount: number) => void }) {
  const source = bout.media?.objectKey ? bout.media.playbackUrl : bout.media?.externalUrl?.match(/\.(mp4|webm|mov)(\?|$)/i) ? bout.media.externalUrl : null;
  ...
  return <div className="video-stage">
    <div className="video-viewport">{source ? <video ref={videoRef} src={source} playsInline onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} onEnded={() => setIsPlaying(false)} onTimeUpdate={(event) => onTime(event.currentTarget.currentTime * 1_000)} onLoadedMetadata={(event) => { if (currentMs) event.currentTarget.currentTime = currentMs / 1_000; }} /> : <div className="video-placeholder"><div className="piste"><i /><span className="fencer-silhouette left">◢</span><span className="fencer-silhouette right">◣</span></div><strong>{pendingTitle}</strong><p>{pendingDetail}</p>{bout.sourceUrl && <a href={bout.sourceUrl} target="_blank" rel="noreferrer">Open FencingTV source ↗</a>}</div>}</div>
...
function PhraseCallForm({ phrase, onSaved, onDeleted }: { phrase: PhraseRecord; onSaved: () => void; onDeleted: () => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setError(''); const form = new FormData(event.currentTarget); const award = form.get('award') as PhraseInput['award']; const before = { left: Number(form.get('leftBefore')), right: Number(form.get('rightBefore')) }; const after = { left: Number(form.get('leftAfter')), right: Number(form.get('rightAfter')) }; const input: PhraseInput = { startMs: Number(form.get('startMs')), endMs: Number(form.get('endMs')), startFrame: phrase.startFrame, endFrame: phrase.endFrame, startReason: form.get('startReason') as PhraseInput['startReason'], endReason: form.get('endReason') as PhraseInput['endReason'], haltReason: String(form.get('haltReason') || ''), award, callStatus: form.get('callStatus') as PhraseInput['callStatus'], callExplanation: String(form.get('callExplanation') || ''), ruleRefs: refs(String(form.get('ruleRefs') || '')), reviewState: form.get('reviewState') as PhraseInput['reviewState'], scoreBefore: before, scoreAfter: after }; try { await api.updatePhrase(phrase.id, input); onSaved(); } catch (caught) { setError(caught instanceof Error ? caught.message : 'Could not save call'); setBusy(false); } }
...
const jointOrder = ['nose', 'weapon_shoulder', 'weapon_elbow', 'weapon_wrist', 'front_hip', 'rear_hip', 'front_knee', 'rear_knee', 'front_ankle', 'rear_ankle', 'guard', 'blade_mid', 'tip'];
const skeletonLines: Array<[string, string]> = [['nose', 'weapon_shoulder'], ['weapon_shoulder', 'weapon_elbow'], ['weapon_elbow', 'weapon_wrist'], ['weapon_shoulder', 'front_hip'], ['front_hip', 'rear_hip'], ['front_hip', 'front_knee'], ['front_knee', 'front_ankle'], ['rear_hip', 'rear_knee'], ['rear_knee', 'rear_ankle'], ['weapon_wrist', 'guard'], ['guard', 'blade_mid'], ['blade_mid', 'tip']];

function PoseEditor({ bout, currentMs, onSaved }: { bout: BoutDetail; currentMs: number; onSaved: () => void }) {
  const [side, setSide] = useState<'left' | 'right'>('left'); const [activeJoint, setActiveJoint] = useState(jointOrder[0]); const [points, setPoints] = useState<Record<string, { x: number; y: number }>>({}); const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  const relevant = bout.poses.filter((pose) => Math.abs(pose.timestampMs - currentMs) < 250);
  function place(event: MouseEvent<HTMLDivElement>) { const rect = event.currentTarget.getBoundingClientRect(); setPoints((current) => ({ ...current, [activeJoint]: { x: (event.clientX - rect.left) / rect.width, y: (event.clientY - rect.top) / rect.height } })); const next = jointOrder[jointOrder.indexOf(activeJoint) + 1]; if (next) setActiveJoint(next); }
  async function save(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setError(''); const form = new FormData(event.currentTarget); const point = (name: string) => points[name] ? { name, ...points[name], visible: true, confidence: 1 } : null; const weapon = { guard: point('guard'), bladeMid: point('blade_mid'), tip: point('tip') }; const keypoints = jointOrder.filter((name) => !['guard', 'blade_mid', 'tip'].includes(name)).map(point).filter((item): item is NonNullable<typeof item> => Boolean(item)); const input: PoseKeyframeInput = { timestampMs: Math.round(currentMs), frameNumber: Math.round(currentMs * (bout.media?.fps || 60) / 1_000), side, trackId: `${side}-1`, keypoints, weapon, bbox: null, frontFootMeters: form.get('frontFoot') ? Number(form.get('frontFoot')) : null, rearFootMeters: form.get('rearFoot') ? Number(form.get('rearFoot')) : null, opponentDistanceMeters: form.get('distance') ? Number(form.get('distance')) : null, occluded: form.get('occluded') === 'on', source: 'human' }; try { await api.savePose(bout.id, input); setPoints({}); onSaved(); } catch (caught) { setError(caught instanceof Error ? caught.message : 'Could not save pose'); setBusy(false); } }
  return <form className="pose-editor" onSubmit={save}><div className="pose-toolbar"><div className="side-toggle"><button type="button" className={side === 'left' ? 'active' : ''} onClick={() => setSide('left')}>Left</button><button type="button" className={side === 'right' ? 'active' : ''} onClick={() => setSide('right')}>Right</button></div><span>{relevant.length} pose{relevant.length === 1 ? '' : 's'} near this frame</span></div><p className="pose-help">Select a point, then click its location. Coordinates are normalized, so labels survive transcoding.</p><div className="joint-picker">{jointOrder.map((joint) => <button type="button" className={`${activeJoint === joint ? 'active' : ''} ${points[joint] ? 'placed' : ''}`} onClick={() => setActiveJoint(joint)} key={joint}>{points[joint] ? '✓ ' : ''}{label(joint.replaceAll('_', '-'))}</button>)}</div><div className="pose-canvas" onClick={place}><div className="pose-piste" />
    <svg viewBox="0 0 100 100" preserveAspectRatio="none">{skeletonLines.map(([from, to]) => points[from] && points[to] ? <line key={`${from}-${to}`} x1={points[from].x * 100} y1={points[from].y * 100} x2={points[to].x * 100} y2={points[to].y * 100} /> : null)}</svg>{Object.entries(points).map(([name, point]) => <i className={name.startsWith('blade') || name === 'tip' || name === 'guard' ? 'weapon-point' : ''} key={name} style={{ left: `${point.x * 100}%`, top: `${point.y * 100}%` }} title={name}><span>{jointOrder.indexOf(name) + 1}</span></i>)}<strong>Click to place: {label(activeJoint.replaceAll('_', '-'))}</strong></div><div className="form-row"><label>Front foot (m)<input name="frontFoot" type="number" step="0.01" /></label><label>Rear foot (m)<input name="rearFoot" type="number" step="0.01" /></label><label>Opponent distance (m)<input name="distance" type="number" min="0" step="0.01" /></label></div><label className="check-label"><input name="occluded" type="checkbox" /> Fencer or weapon is meaningfully occluded</label>{error && <p className="field-error">{error}</p>}<button className="primary-button full-button" disabled={busy || Object.keys(points).length === 0}>{busy ? 'Saving…' : `Save ${side} pose at ${formatTime(currentMs)}`}</button></form>;
}
